...
(Previously: deterministic ordering covered ecosystems, managers, manifests, lockfiles, workspaces, wrappers, framework signals, and build/test-tool signals; no suffix rules or output-dir signals.)
...
(Previously: only Go, JavaScript, and Python ecosystems were evidenced.)
...
(Previously: only node_modules, vendor, target, .venv, bin, and obj were excluded.)
...
(Previously: families were limited to JS/Python/Go evidence and pdm.lock was not recognized.)
