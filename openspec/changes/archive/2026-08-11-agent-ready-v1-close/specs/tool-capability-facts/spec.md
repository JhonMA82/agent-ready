# Delta for Tool Capability Facts
...
(Previously: no safety-level, install-method, side-effect, or integration metadata existed in the contract.)
...
(Previously: signals were limited to dist/build/coverage output directories and a minimal lockfile list.)
...
(Previously: this change deferred safe recipes, system-package-manager coverage, provider lifecycle, and global OpenCode/RTK integration.)

#### Scenario: Unsupported capability is claimed

- GIVEN an entry claims install support without tests covering detect, version, plan, execute, and verify
