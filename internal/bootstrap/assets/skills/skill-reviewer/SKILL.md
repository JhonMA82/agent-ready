...
- Run the placement checks: `context_savings` (the move reduces permanent context without losing guidance), `duplication_after_extraction` (content must not remain duplicated in the source), `discoverability_preserved` (critical rules stay discoverable), and `always_on_guidance_not_removed` (a global invariant must not be moved into a skill). REJECT an extraction that fails one of them.
- Reject a technically excellent skill when the same content remains fully in AGENTS: no progressive disclosure, no real context saving.
...
5. Apply the placement checks to extraction candidates; a failure is a blocking concern.
6. Score each rubric criterion, including context_placement; write the score sheet.
7. Return PASS, REVISE, or REJECT; record the sheet in state.
